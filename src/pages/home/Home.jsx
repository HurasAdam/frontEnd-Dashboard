...
if(data){
    ...
    const tickets = data.filter((ob)=>ob.name==='ticketsOpen'|| ob.name==='ticketsClosed'||ob.name==='totalTickets')
    const type = data.filter((ob)=>ob.name==='Bug'|| ob.name==='Enhancement'||ob.name==='Question')
    ...
    return(
        <div className="home" id={theme.mode}>
         <FeaturedInfo data={stats&&stats.tickets.reduce((obj, item) => {
    ...
    return obj;
        ...
        <CircleChart data={stats&&stats.type} title='Report types'></CircleChart>
         </div>
